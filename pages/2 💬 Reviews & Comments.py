...
with st.form("filtering"):
    ...
    property_review = df3['listing_id'].unique()
    cleanliness = 'clean|tidy|neat|stain|trash|rubbish|dirt|dust'
...
if searched:
    if property_id:
        if int(property_id) in property_review:
            selected_comment = df3[df3['listing_id'] == int(property_id)][['listing_id','date','reviewer_name','comments']]
            cmt_cleanliness = selected_comment[selected_comment['comments'].str.contains(cleanliness, na=False, flags=re.IGNORECASE, regex=True)]
            num_cmt_cleanliness = len(cmt_cleanliness)
            ...
            with col3:
                st.caption(f"Property Location: {location_score} {star_location}")
            with col4:
                st.caption(f"Property Value: {value_score} {star_value}")

            st.subheader(f"Comments about Cleanliness: {num_cmt_cleanliness}")
            st.dataframe(cmt_cleanliness)
        else:
            st.markdown("There is no comment about your selected property")
    else:
        st.markdown("You have not chosen any property to investigate")
